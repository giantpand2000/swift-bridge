...
fn normalize_extern_swift_items(tokens: TokenStream) -> syn::Result<(TokenStream, bool)> {
    ...
    for token in tokens {
        ...
        if is_semi {
            let (item, item_changed) = normalize_extern_swift_item(current_item)?;
...
fn normalize_extern_swift_item(tokens: TokenStream) -> syn::Result<(TokenStream, bool)> {
    if tokens.is_empty() {
        return Ok((tokens, false));
    }

    match syn::parse2::<SwiftFunc>(tokens.clone()) {
        Ok(swift_func) => Ok((swift_func.to_rust_foreign_fn(), true)),
        Err(err) => {
...
impl SwiftFunc {
    fn to_rust_foreign_fn(&self) -> TokenStream {
